...
const FromItem = require("./FromItem");
const GroupByElement = require("./GroupByElement");
const WindowItem = require("./WindowItem");
const OrderByElement = require("./OrderByElement");
const Union = require("./Union");
const SelectFetch = require("./SelectFetch");
...
class Select extends Syntax {
    ...
    static parseColumns(coach, data) {
        if ( !coach.isColumn() ) {
            return;
        }
    ...
    static parseGroupBy(coach, data) {
    ...
    /*
        ...
        [ FETCH { FIRST | NEXT } [ count ] { ROW | ROWS } ONLY ]
     */
    static parseOffsets(coach, data) {
    ...
    // [ FETCH { FIRST | NEXT } [ count ] { ROW | ROWS } ONLY ]
    static parseFetch(coach, data) {
        if ( !coach.isSelectFetch() ) {
            return;
        }

        data.fetch = coach.isParseFetch();
        return true;
    }

    // [ ORDER BY expression [ ASC | DESC | USING operator ] [ NULLS { FIRST | LAST } ] [, ...] ]
    static parseOrderBy(coach, data) {
